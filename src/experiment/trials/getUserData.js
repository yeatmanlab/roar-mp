...
const getLabId = {
  ...
  const ifGetLabId = {
    ...
    conditional_function: () => {
      if (store.session.get('config').taskVariant === "otherLabs") {
  ...
  const ifGetPid = {
    ...
    conditional_function: function () {
      return (store.session.get('config').pid === null && store.session.get('config').userMode !== "demo");
    }
  };
...
// conditional timeline for consent form: will only appear if user mode or task variant = demo
  const ifConsentForm = {
    ...
    conditional_function: () => {
      return Boolean((store.session.get('config').userMode === "demo") || ((store.session.get('config').taskVariant === 'otherLabs') && (store.session.get('config').consent === true)));
    },
  };
...
export const getUserDataTimeline = [enterFullscreen, ifGetLabId, ifGetPid, ifConsentForm, ifSurvey, ifGradeSelect ]